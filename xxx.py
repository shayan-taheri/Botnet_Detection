...
with tf.variable_scope('model'):
    ...
    with tf.variable_scope('train_op'):
        ...
        optimizer = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.9, use_locking=False, name='Momentum')
        env.train_op = optimizer.minimize(env.loss)
...
def evaluate(sess, env, X_data, y_data, batch_size=1):
    """
...
def train(sess, env, X_data, y_data, X_valid=None, y_valid=None, epochs=1,
    ...
    for epoch in range(epochs):
        ...
        if (epochs == 60) | (epochs == 120) | (epochs == 160):
...
def predict(sess, env, X_data, batch_size=1):
    """
...
def make_deepfool(sess, env, X_data, epochs=1, batch_size=128):
    ...
    for batch in range(n_batch):
        ...
        adv = sess.run(env.x_deepfool, feed_dict={env.x: X_data[start:end],
...
train(sess, env, X_train, y_train, X_valid, y_valid, load=False, epochs=1, learning_rate=0.1,
      name='mnist')
...
xadvs = [make_fgsm(sess, env, xorg, eps=0.1, epochs=1),
make_jsma(sess, env, xorg, eps=0.1, epochs=1),
